...
public class OCCTSplitModelSelection extends ModelSelection {
    ...
    /** The attributes which can be used for split **/
    private List<Attribute> m_possibleAttributes;
    private List<Attribute> m_attributesOfB;

    private boolean m_wasRan;
    private int m_selectedSplitAttributeIndex;
    ...
    public OCCTSplitModelSelection(String splitCriterionType, Instances allData,
        ...
        while (attributes.hasMoreElements()) {
            this.m_possibleAttributes.add((Attribute)attributes.nextElement());
        }
        this.m_attributesOfB = attributesOfB;
        this.m_wasRan = false;
        this.m_selectedSplitAttributeIndex = -1;
    }
    ...
    private ClassifierSplitModel updateLastRunAndReturnBestModel(
                Set<OCCTSingleAttributeSplitModel> allBestModels) {
        this.m_wasRan = true;
        OCCTSingleAttributeSplitModel bestModel = allBestModels.iterator().next();
        this.m_selectedSplitAttributeIndex = bestModel.getSplittingAttributeIndex();
        return bestModel;
    }
    ...
    /**
     * Selects the relevant split for the given dataset
     */
    ...
    /**
     *
     * @return The attribute chosen for split at LAST time this model selection method was ran
     *
     * @throws IllegalStateException In case the model wasn't ran (or cleanup method was called
     *                               without further next runs)
     */
    public Attribute getChosenAttribute() throws IllegalStateException {
        if (!this.m_wasRan) {
            throw new IllegalStateException("The model selection still was not applied");
        }
        return this.m_allData.attribute(this.m_selectedSplitAttributeIndex);
    }
